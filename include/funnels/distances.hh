...
//  }
  ...
  MatrixXd & cp_vv(const VectorXd & v0, const VectorXd & v1){
    _tmpMat = v0-v1;
    do_so2_wrap();
    return _tmpMat;
  }
  
  MatrixXd & cp_vM(const VectorXd & v0, const MatrixXd & m1){
    _tmpMat = (-m1).colwise() + v0;
    do_so2_wrap();
    return _tmpMat;
  }
  
  MatrixXd & cp_Mv(const MatrixXd & m0, const VectorXd & v1){
    _tmpMat = m0.colwise() - v1;
    do_so2_wrap();
    return _tmpMat;
  }
  
  MatrixXd & cp_MM(const MatrixXd & m0, const MatrixXd & m1){
    _tmpMat = m0 - m1;
    do_so2_wrap();
    return _tmpMat;
  }
